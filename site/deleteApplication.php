...
if (isset($_GET['Application_ID'])) {
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            $stmt->execute([$_GET['Application_ID']]);
            $msg = 'You have deleted the Application!';
        } else {
            // User clicked the "No" button, redirect to the read page
...
<div class="back" <input type="submit" value="Back" onclick="window.location.href='manageApplication.php'"><span>Back</span></div>
	<h2>Delete Application #'<?=$Application['Application_ID']?>'</h2>
    <?php if ($msg): ?>
    <p><?=$msg?></p>
    <?php else: ?>
    <div class="delete-message">
	<p>You are about to delete the application #<?=$Application['Application_ID']?>.<br/> Continue ? </p>
    </div>
    <div class="yesno">
        <a href="deleteApplication.php?Application_ID=<?=$Application['Application_ID']?>&confirm=yes">Yes</a>
        <a href="deleteApplication.php?Application_ID=<?=$Application['Application_ID']?>&confirm=no">No</a>
    </div>
    <?php endif; ?>
</div>
</main>
